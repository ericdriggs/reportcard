...
CREATE TABLE IF NOT EXISTS `reportcard`.`job` (
  ...
  PRIMARY KEY (`job_id`),
  UNIQUE INDEX `UQ_JOB_BRANCH_SHA_INFO` (`sha_fk` ASC, `branch_fk` ASC, `job_info_str` ASC) VISIBLE,
  INDEX `FK_JOB_BRANCH` (`branch_fk` ASC) VISIBLE,
  INDEX `FK_JOB_SHA` (`sha_fk` ASC) VISIBLE,
  CONSTRAINT `FK_JOB_BRANCH`
    FOREIGN KEY (`branch_fk`)
    REFERENCES `reportcard`.`branch` (`branch_id`),
  CONSTRAINT `FK_JOB_SHA`
    FOREIGN KEY (`sha_fk`)
...
CREATE TABLE IF NOT EXISTS `reportcard`.`execution` (
  ...
  INDEX `execution_context_fk_idx` (`job_fk` ASC) VISIBLE,
