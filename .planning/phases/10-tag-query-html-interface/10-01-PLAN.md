...
<task type="auto">
  ...
  <action>
    ...
    3. Five endpoints matching TagQueryController paths (without /api/v1):
    ...
    4. Each method:
       ...
       - If tags present:
         * Try calling tagQueryService.findByTagExpressionByPath()
         * Build TagQueryResponse with QueryInfo containing scope and tags
         * Return results via TagQueryHtmlHelper.renderTagQueryPage(response, scopePath)
       - Catch ParseException and return error page via TagQueryHtmlHelper.renderErrorPage()
       - Return ResponseEntity<String> with appropriate HttpStatus (OK or BAD_REQUEST)

    Follow BrowseUIController pattern for consistency.
  </action>
  <verify>Compile with: ./gradlew :reportcard-server:compileJava</verify>
...
<task type="auto">
  ...
  <action>
    ...
    1. In getCompanyLinks() method (around line 58):
       ...
       - Add replace: .replace("{tagSearchLink}", getLink("Tag Search 🔍", "/company/" + company + "/tags/tests"))
       - Include <br> between links for proper spacing

    2. In getOrgLinks() method (around line 117):
       - Add new line after {metricsLink}
       - Add placeholder: {tagSearchLink}
       - Add replace: .replace("{tagSearchLink}", getLink("Tag Search 🔍", orgPath.toUrlPath() + "/tags/tests"))
       - Include <br> between links for proper spacing

    Use emoji 🔍 for visual consistency with existing Metrics link (🔢).
